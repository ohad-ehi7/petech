<x-header :title="'Home'">
    ...
    <!-- Profile section -->
    <div class="relative flex items-center p-4 border border-gray-200 shadow bg-cover bg-center overflow-hidden"
        style="background-color: rgba(255, 255, 255, 0.8);">
        <!-- Background layer -->
        <div class="absolute inset-0 bg-cover bg-center opacity-40"
            style="background-image: url('images/small-background.png'); z-index: 0;"></div>

        <!-- Foreground content -->
        <div class="relative z-10 flex items-center">
            <img src="images/petech.png" alt="Picture nako"
                class="w-12 h-12 rounded-full border-2 border-gray-300 dark:border-gray-600 mr-4">
            <div class="flex flex-col">
                <h2 class="text-lg font-semibold text-black">Hello, {{ Auth::user()->name }}</h2>
                <p class="text-black">PETECH-POS</p>
            </div>
        </div>
    </div>
        ...
        <div class="col-span-3">
            <div class="max-w-3xl mx-auto rounded-lg overflow-hidden shadow border border-gray-200">
                ...
                <div class="bg-white grid grid-cols-4 text-center divide-x divide-gray-200 p-4">
                    <!-- Today Sales -->
                    <div>
                        <div class="text-2xl font-bold text-blue-500">HTG {{ number_format($todaySales, 2) }}</div>
                        <div class="text-gray-500 text-sm">Gourdes</div>
            ...
            <div class="rounded-lg shadow-md overflow-hidden">
                ...
                <div class="bg-white p-4 flex justify-between items-center">
                    ...
                    <div class="flex flex-col items-center justify-center text-xs text-gray-400">
                        ...
                        <div
                            class="w-20 h-20 flex items-center justify-center rounded-full border-[6px] border-gray-200 text-center">
                            <span
                                class="text-[10px] text-center leading-tight">{{ $inventorySummary['active_items'] }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-lg shadow-md overflow-hidden">
                ...
                <div class="bg-amber-400 flex justify-between items-center">
                    ...
                    <button id="resetButton"
                        class="px-3 py-1 bg-gray-200 text-red-700 rounded hover:bg-gray-300 text-sm">
                        Reset
                    </button>
                    <form method="GET" action="{{ route('home') }}" class="mr-4">
                        <select name="date_range" class="text-sm text-gray-800 bg-transparent border-none focus:ring-0"
                            onchange="this.form.submit()">
                            <option value="today" {{ $dateRange === 'today' ? 'selected' : '' }}>Today</option>
                            <option value="yesterday" {{ $dateRange === 'yesterday' ? 'selected' : '' }}>Yesterday
                            </option>
                            <option value="this_week" {{ $dateRange === 'this_week' ? 'selected' : '' }}>This Week
                            </option>
                            <option value="last_week" {{ $dateRange === 'last_week' ? 'selected' : '' }}>Last Week
                            </option>
                            <option value="this_month" {{ $dateRange === 'this_month' ? 'selected' : '' }}>This Month
                            </option>
                            <option value="last_month" {{ $dateRange === 'last_month' ? 'selected' : '' }}>Last Month
                            </option>
                            <option value="this_year" {{ $dateRange === 'this_year' ? 'selected' : '' }}>This Year
                            </option>
                            <option value="last_year" {{ $dateRange === 'last_year' ? 'selected' : '' }}>Last Year
                            </option>
                        </select>
                    </form>
                </div>
                ...
                <div class="bg-white p-4">
                    <table id="topSellingTable" class="min-w-full divide-y divide-gray-200">
                            ...
                            @forelse($topSellingItems as $item)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ $item->ProductName }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ (int) $item->total_quantity }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">HTG
                                        {{ number_format($item->total_sales ?? 0, 2) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ $item->CategoryName ?? 'N/A' }}</td>
                                </tr>
                            @empty
                                <!-- Ligne vide avec 4 colonnes pour DataTables -->
                                <tr>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td class="text-center text-sm text-gray-500">No items were invoiced in this time
                                        frame</td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="col-span-1">
                ...
                <div class="bg-white rounded-lg shadow-lg">
                    ...
                    <div class="flex flex-col items-center justify-center px-4 py-6 space-y-4">
                        ...
                        <div class="text-xl text-blue-500 font-bold">HTG
                            {{ number_format(
                        ...
                        <div class="text-lg text-blue-500 font-semibold">HTG
                            {{ number_format($products->avg('CostPrice'), 2) }}</div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="flex justify-between items-center bg-sky-300 rounded-t-lg">
                        ...
                        <form method="GET" action="{{ route('home') }}" class="mr-4">
                            <select name="date_range"
                                class="text-sm text-black bg-transparent border-none focus:ring-0"
                                onchange="this.form.submit()">
                                <option value="today" {{ $dateRange === 'today' ? 'selected' : '' }}>Today</option>
                                <option value="yesterday" {{ $dateRange === 'yesterday' ? 'selected' : '' }}>Yesterday
                                </option>
                                ...
                                <option value="this_month" {{ $dateRange === 'this_month' ? 'selected' : '' }}>This
                                    Month</option>
                                <option value="last_month" {{ $dateRange === 'last_month' ? 'selected' : '' }}>Last
                                    Month</option>
                                <option value="this_year" {{ $dateRange === 'this_year' ? 'selected' : '' }}>This Year
                                </option>
                                <option value="last_year" {{ $dateRange === 'last_year' ? 'selected' : '' }}>Last Year
            ...
            <div class="col-span-5">
                <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    <div class="flex justify-between items-center bg-indigo-200 rounded-t-xl">
                        <h2 class="text-sm font-semibold text-gray-800 p-4">Sales Order Summary</h2>
                        <!-- Bouton Reset -->
                        ...
                        <div class="md:w-48 mt-6 md:mt-0 md:ml-6">
                            ...
                            <div class="flex items-center bg-blue-50 border border-blue-200 rounded-md px-3 py-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-lg font-semibold text-gray-700">HTG
                                        {{ number_format($monthlyTotal, 2) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            if ($('#topSellingTable tbody tr').length) {
                $('#topSellingTable').DataTable({
                    dom: '<"flex flex-wrap items-center justify-between"<"flex items-center"><"flex items-center"f>>rtip',
                    pageLength: 15,
                    order: [
                        [0, 'desc']
                    ],
        ...
        document.getElementById('resetButton').addEventListener('click', function() {
            // Rediriger vers la route home sans paramètres
